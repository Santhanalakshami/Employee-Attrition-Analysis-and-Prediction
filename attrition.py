...
feature_columns = joblib.load("attrition_feature_columns.joblib")  # Load saved feature columns list

# ===== Streamlit app =====
st.title(':red[Employee Attrition Prediction]')

# Sidebar for model selection
model_type = st.sidebar.selectbox('Select Model', ['Logistic Regression', 'Random Forest'])

# ===== Input form =====
st.header(':green[Employee Info]')
...
if st.button('Predict Attrition Risk'):
    if model_type == 'Logistic Regression':
        prediction = log_reg_loaded.predict(input_df)[0]
        probability = log_reg_loaded.predict_proba(input_df)[0][1]
    else:
        prediction = rf_loaded.predict(input_df)[0]
        probability = rf_loaded.predict_proba(input_df)[0][1]
